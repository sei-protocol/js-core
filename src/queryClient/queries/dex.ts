...
export function setupDexExtension(base: QueryClient) {
	...
	return {
		dex: {
			getPrices: async (contractAddr: string, priceDenom: string, assetDenom: string): Promise<QueryGetPricesResponse> => {
			...
			shortBookAll: async (contractAddr: string, priceDenom: string, assetDenom: string, pagination?: any): Promise<QueryAllShortBookResponse> => {
				const { ShortBook, pagination: paginationResponse } = await queryService.ShortBookAll({
					contractAddr,
					priceDenom,
					assetDenom,
					pagination
			...
			longBookAll: async (contractAddr: string, priceDenom: string, assetDenom: string, pagination?: any): Promise<QueryAllLongBookResponse> => {
				const { LongBook, pagination: paginationResponse } = await queryService.LongBookAll({
					contractAddr,
					priceDenom,
					assetDenom,
					pagination
			...
			getMarketSummary: async (contractAddr: string, priceDenom: string, assetDenom: string, lookbackInSeconds: number): Promise<QueryGetMarketSummaryResponse> => {
				...
				return {
					...
					totalVolumeNotional: formatAmount(marketSummary.totalVolumeNotional, RPC_UNITS)
				};
			},
			getOrders: async (contractAddr: string, account: string): Promise<QueryGetOrdersResponse> => {
